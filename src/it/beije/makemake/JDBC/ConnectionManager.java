	...
	public static Connection getConnection() throws ClassNotFoundException, SQLException {
		...
		for(Connection connection : connectionList) {
			if(connection.isClosed()) {
				connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/makemake?serverTimezone=CET", "root", "Beije02");
				return connection;
			}
		}
		...
		if(connectionList.size()<CONNECTION_LIMIT) {
			...
			connectionList.add( DriverManager.getConnection("jdbc:mysql://localhost:3306/makemake?serverTimezone=CET", "root", "Beije02"));
