...
public class Ecommerce {
		...
		try {
			...
			if(orderList.isEmpty()) {
				...
				for(OrderItem o : orderItems) {
					...
					System.out.println("Il product_id corrisponde al seguente prodotto : "+ productList.get(0).getName()+", "+
							productList.get(0).getBrand()+", "+productList.get(0).getDescription()+", "+productList.get(0).getPrice());
				}
				System.out.println("-------------------------------------------------------------------------------------------------");
				System.out.println("-------------------------------------------------------------------------------------------------");
			entityManager.close();
	...
	private static void orderCreation() {
		...
		Query userQuery = entityManager.createNativeQuery("SELECT * FROM User WHERE username = :username AND password = : psw", User.class);
